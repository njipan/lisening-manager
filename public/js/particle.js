...
class Particle {
    ...
    draw(ctx) {
        ...
        if (mag < 150) {
            ctx.strokeStyle = 'rgba(255, 255, 255, ' + ((150 - mag) / 300) + ')';
            this.drawLine(ctx, this.position, this.mousePosition);
            //this.moveTo(this.mousePosition);
            ctx.fillStyle = 'rgba(255, 255, 255, '+ (0.5 + ((150 - mag) / 300)) +')';
        } else {
            ...
            for (var j = Math.max(col - 1, 0) ; j <= col + 1; j++) {
                ...
                this.particleMap[i][j].forEach(particle => {
                    ...
                    if (mag < this.chunkSize.x) {
                        ctx.strokeStyle = 'rgba(255, 255, 255, '+ ((this.chunkSize.x - mag) / this.chunkSize.x) + ')';
